/**
 * Provision Jeddah Technologies client portal external user.
 *
 *   https://greendesert.unit311central.com/jeddahtechnologies
 *   jeddahtechnologies@greendesert.unit311central.com / Reactor20206$
 *
 *   node scripts/provision-greendesert-jeddah-portal.mjs
 */
...
const SLUG = "greendesert";
const PASSWORD = "Reactor20206$";
const CLIENT_ID = "greendesert-cli-jeddah-technologies";
const USERNAME = "jeddahtechnologies@greendesert.unit311central.com";
const PORTAL_PATH = "jeddahtechnologies";
const PORTAL_URL = `https://greendesert.unit311central.com/${PORTAL_PATH}`;
...
async function main() {
  const { data: ws, error: wsErr } = await admin
    ...
    .maybeSingle();
  if (wsErr || !ws) {
    console.error("Green Desert workspace not found", wsErr?.message);
    process.exit(1);
  }
  ...
  if (!existingClient?.id) {
    const { error: insertClientErr } = await admin.from("internal_clients").insert({
      id: CLIENT_ID,
      workspace_id: ws.id,
      company_name: "Jeddah Technologies",
      ...
      notes: "Green Desert external client portal — Jeddah Technologies",
      platform_url: PORTAL_URL,
      email: USERNAME,
      primary_contact: "Jeddah Technologies Portal Admin",
      created_at: now,
      updated_at: now,
    });
    if (insertClientErr) {
      console.error("Failed to create client", insertClientErr.message);
      process.exit(1);
    }
