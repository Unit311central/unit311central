/**
 * Provision Green Desert Board Portal external user.
 *
 *   https://greendesert.unit311central.com/board
 *   board@greendesert.unit311central.com / Algae2026$
 *
 *   node scripts/provision-greendesert-board-portal.mjs
 */
...
const PASSWORD = "Algae2026$";
const CLIENT_ID = "greendesert-cli-board";
const USERNAME = "board@greendesert.unit311central.com";
...
async function main() {
  ...
  if (!existingClient?.id) {
    const { error: insertClientErr } = await admin.from("internal_clients").insert({
      id: CLIENT_ID,
      workspace_id: ws.id,
      company_name: "Green Desert Board",
      account_status: "Active",
      industry: "Governance",
      region: "United States",
      company_country: "United States",
      contract_type: "Board Access",
      notes: "Green Desert Board Portal — external board member access",
      platform_url: PORTAL_URL,
      email: USERNAME,
      primary_contact: "Green Desert Board Member",
      created_at: now,
      updated_at: now,
    });
    if (insertClientErr) {
