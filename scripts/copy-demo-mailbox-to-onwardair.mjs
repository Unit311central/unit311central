/**
 * Copy demo@unit311central.com Zoho credentials from Demo (or ABHI) onto OnwardAir.
 *
 *   node scripts/copy-demo-mailbox-to-onwardair.mjs
 */
...
const SUPABASE_URL = merged.SUPABASE_URL || merged.NEXT_PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL;
const SERVICE_KEY = merged.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
...
const SOURCE_SLUGS = ["demo", "abhi"];
const TARGET_SLUG = "onwardair";
...
async function main() {
  ...
  // Fall back to env Zoho demo password if no DB row.
  const envPassword =
    merged.ZOHO_DEMO_PASSWORD ||
    merged.ZOHO_PASSWORD ||
    merged.ZOHO_APP_PASSWORD ||
    process.env.ZOHO_DEMO_PASSWORD ||
  ...
  if (!password) {
    throw new Error("No demo mailbox password found in Demo/ABHI DB or ZOHO_* env");
  }

  console.log(
    sourceCred
      ? `Using password from ${sourceSlug} DB row (${email})`
      : `Using ZOHO env password for ${email}`,
  );
  ...
  console.log("Upserted OnwardAir demo mailbox credentials:", data);
}
