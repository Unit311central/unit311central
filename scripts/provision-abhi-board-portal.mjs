/**
 * Provision ABHI Board Portal external user.
 *
 *   https://abhi.unit311central.com/board
 *   board@abhi.org.uk / London1999$
 *
 *   node scripts/provision-abhi-board-portal.mjs
 */
...
const CLIENT_ID = "abhi-cli-board";
const USERNAME = "board@abhi.org.uk";
const PORTAL_PATH = "board";
const PORTAL_URL = `https://abhi.unit311central.com/${PORTAL_PATH}`;
...
async function main() {
  ...
  if (!existingClient?.id) {
    const { error: insertClientErr } = await admin.from("internal_clients").insert({
      id: CLIENT_ID,
      workspace_id: ws.id,
      company_name: "ABHI Board",
      account_status: "Active",
      industry: "Governance",
      region: "United Kingdom",
      company_country: "United Kingdom",
      contract_type: "Board Access",
      notes: "ABHI Board Portal — external board member access (role: Board Member)",
      platform_url: PORTAL_URL,
      email: USERNAME,
      primary_contact: "ABHI Board Member",
      created_at: now,
      updated_at: now,
    });
    if (insertClientErr) {
  ...
  const payload = {
    username,
    display_name: "ABHI Board Member",
    user_type: "external",
    workspace_id: ws.id,
    client_id: CLIENT_ID,
    client_name: "ABHI Board",
    email: username,
