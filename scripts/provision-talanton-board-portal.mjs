/**
 * Provision Talanton Impact Board Portal external user.
 *
 *   https://talantonimpact.unit311central.com/board
 *   board@talantonimpact.com / Africa1999$
 *
 *   node scripts/provision-talanton-board-portal.mjs
 */
...
const CLIENT_ID = "ti-cli-board";
const USERNAME = "board@talantonimpact.com";
const PORTAL_PATH = "board";
const PORTAL_URL = `https://talantonimpact.unit311central.com/${PORTAL_PATH}`;
...
async function main() {
  const { data: ws, error: wsErr } = await admin
    ...
    .maybeSingle();
  if (wsErr || !ws) {
    console.error("Talanton Impact workspace not found", wsErr?.message);
    process.exit(1);
  }
  ...
  if (!existingClient?.id) {
    const { error: insertClientErr } = await admin.from("internal_clients").insert({
      id: CLIENT_ID,
      workspace_id: ws.id,
      company_name: "Talanton Impact Board",
      ...
      notes:
        "Talanton Impact Board Portal — external board member access (role: Board Member)",
      platform_url: PORTAL_URL,
      email: USERNAME,
      primary_contact: "Talanton Impact Board Member",
      created_at: now,
      updated_at: now,
    });
    if (insertClientErr) {
  ...
  const payload = {
    username,
    display_name: "Talanton Impact Board Member",
    user_type: "external",
    workspace_id: ws.id,
    client_id: CLIENT_ID,
    client_name: "Talanton Impact Board",
    email: username,
