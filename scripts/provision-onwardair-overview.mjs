/**
 * Provision OnwardAir Overview invite page external user.
 *
 *   https://onwardair.unit311central.com/overview
 *   overview@onwardair.tech / Overview4141&
 *
 *   node scripts/provision-onwardair-overview.mjs
 */
...
const CLIENT_ID = "oa-cli-overview";
const USERNAME = "overview@onwardair.tech";
const PORTAL_PATH = "overview";
...
async function main() {
  ...
  if (!existingClient?.id) {
    const { error: insertClientErr } = await admin.from("internal_clients").insert({
      id: CLIENT_ID,
      workspace_id: ws.id,
      company_name: "OnwardAir Overview",
      account_status: "Active",
      industry: "Aerospace",
      region: "United States",
      company_country: "United States",
      company_city: "Houston",
      contract_type: "Overview Access",
      notes: "Private Scott invite overview page — not full platform access",
      platform_url: PORTAL_URL,
      email: USERNAME,
      primary_contact: "Overview Guest",
      created_at: now,
      updated_at: now,
    });
    if (insertClientErr) {
      console.error("Failed to create overview client", insertClientErr.message);
      process.exit(1);
    }
  ...
  const payload = {
    username,
    display_name: "OnwardAir Overview",
    user_type: "external",
    workspace_id: ws.id,
    client_id: CLIENT_ID,
    client_name: "OnwardAir Overview",
    email: username,
