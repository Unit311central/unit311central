-- WOLF Central: enable Analytics module (Platform, System Health, RT Video — no Website Analytics).
...
declare
  ...
  v_enabled_sub_modules jsonb := '[
    "analytics:platform-analytics","analytics:realtime-video-pipeline","analytics:system-health",
  ...
  v_module_keys text[] := array[
    ...
    'file-explorer','logistics','profiles','projects','strategy','support','telemetry','training','users',
...
begin
  ...
  if v_wolf_id is null then
    raise notice '197_wolf_central_analytics_module: wolf-central workspace missing — skipped';
    return;
  end if;
  ...
  raise notice '197_wolf_central_analytics_module: enabled analytics for wolf-central';
end $$;
