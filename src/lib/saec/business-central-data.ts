...
const SAEC_INTERNAL_PROJECTS: InternalProject[] = [
  project({
    id: "saec-int-klk-training",
    name: "KLK range engineering competency programme",
    clientId: null,
    clientName: SAEC_COMPANY_NAME,
    ...
    endDate: "2026-12-31",
    progressPct: 68,
    notes: "Internal lift engineering certification — KLK 450–2000kg range.",
  }),
  project({
    id: "saec-int-safety-qms",
    name: "Vertical transport safety & QMS uplift",
    clientId: null,
    clientName: SAEC_COMPANY_NAME,
    ...
    endDate: "2026-11-30",
    progressPct: 54,
    notes: "ISO-aligned inspection workflows and incident response playbooks.",
  }),
  project({
    id: "saec-int-fleet-digital",
    name: "National service fleet digitisation",
    clientId: null,
    clientName: SAEC_COMPANY_NAME,
    ...
    endDate: "2027-08-31",
    progressPct: 10,
    notes: "GPS routing, parts inventory sync, and engineer dispatch tooling.",
  }),
];
...
export function getSaecFixtureProjects(): InternalProject[] {
  return [...SAEC_INTERNAL_PROJECTS, ...SAEC_PROJECTS].map((row) => ({ ...row }));
}
