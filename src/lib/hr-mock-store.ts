          ...
          reports: [
            {
              ...
              previewLines: [
                ...
                "Segment,Count,Share",
                "Leadership,3,13%",
                "International / Events,6,25%",
                "Digital Health & Regulatory,7,29%",
                "Market Affairs & Access,5,21%",
                "Communications & Ops,3,13%",
              ],
            },
            {
              id: "rep-abhi-2",
              name: "ABHI Performance Summary — H1 2026",
            ...
            {
              id: "rep-abhi-3",
              name: "ABHI Leave Summary — Jul 2026",
              kind: "leave_summary",
              output: "csv",
              filters: {
                ...emptyHrReportFilters(),
              ...
              previewLines: [
                ...
                "Judith Mellis,Annual,5,Approved",
                "Owain Prescott,Sick,3,Approved",
                "Sophie Green,Training,2,Pending",
              ],
            },
          ],
          activity: [
    ...
    try {
      ...
      if (isBrowserDemoSurface()) {
        ...
        return {
          ...
          reviews: fixtures.directory.slice(0, 6).map((row, index) => {
            const ratings = [4, 5, 4, 3, 4, 5] as const;
            return {
              id: `rev-mag-${index + 1}`,
              employeeId: row.id,
              ...
              department: row.department,
              role: row.role,
              managerName: index === 0 ? "Board" : "Reese Sullivan",
              reviewPeriod: "H1 2026",
              status: index % 3 === 0 ? "draft" : "submitted",
              overallRating: ratings[index % ratings.length]!,
              strengths: "Client delivery ownership and clear stakeholder communication.",
              ...
              responses: blankQuestionResponses().map((item, qIndex) => ({
                ...
                rating: ([4, 4, 5, 4, 4, 4, 3, 4, 4, 4, 3, 5, 5, 4, 4, 3, 5, 4, 4, 4][qIndex] ??
                  3) as 1 | 2 | 3 | 4 | 5,
                managerComments: qIndex % 5 === 0 ? "Consistently strong." : "",
                employeeComments: qIndex % 7 === 0 ? "Agree — focusing here next quarter." : "",
              })),
              objectives: [
                {
                  id: `obj-mag-${index}-1`,
                ...
                },
                {
                  ...
                  description: "Publish two reusable playbooks for the practice library.",
                  progressPercent: 55 + (index % 20),
              ...
              competencies: blankCompetencyScores().map((item, cIndex) => ({
                ...item,
                id: `comp-mag-${index}-${cIndex}`,
                score: ([4, 4, 4, 5, 4, 4, 3, 4, 4, 4][cIndex] ?? 4) as 1 | 2 | 3 | 4 | 5,
                notes: cIndex === 0 ? "Trusted by peers" : "",
              })),
              developmentPlan: [
                {
                  id: `dev-mag-${index}`,
                  focus: "Stakeholder leadership",
                  action: "Shadow practice lead on weekly client triage",
                  owner: row.fullName,
              ...
              createdAt: isoDaysFromNow(-(60 + index)),
              updatedAt: isoDaysFromNow(-(10 + index)),
              submittedAt: index % 3 === 0 ? null : isoDaysFromNow(-(8 + index)),
              approvedAt: null,
              completedAt: null,
            };
          }),
          goals: fixtures.directory.slice(0, 8).map((row, index) => ({
            id: `goal-mag-${index + 1}`,
            title: index % 2 === 0 ? "Practice utilisation ≥ 78%" : "Pipeline coverage 3×",
            description:
              index % 2 === 0
                ? "Maintain billable utilisation across consulting pods."
                : "Build qualified pipeline coverage for the next two quarters.",
            progressPercent: 60 + (index % 30),
            dueDate: isoDaysFromNow(90 + index * 5),
            status: "on_track" as const,
            weight: 20,
            owner: row.fullName,
            scope: "employee" as const,
          ...
          activity: [
            {
              ...
              detail: `${fixtures.company.tradingName} leave, recruitment, and performance fixtures.`,
            },
          ],
        };
      }
