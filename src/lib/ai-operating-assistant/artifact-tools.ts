...
export async function emailAssistantArtifact(
  ...
  const workspaceLabel =
    ctx.business.workspace.name?.trim() || "Workspace";
  const to =
    asString(args.to) ||
  ...
  if (!to) {
    return toolError(
      "emailAssistantArtifact",
      "No recipient email was provided. Pass `to` or configure BOARD_EMAIL.",
      [],
    );
  }
  ...
  try {
    await sendMailboxEmail({
      account: "paul",
      to,
      subject: `${artifact.title} — ${workspaceLabel}`,
      text: `Please find attached: ${artifact.title}.\n\nGenerated by the ${workspaceLabel} Executive Assistant.`,
      html: `<p>Please find attached: <strong>${artifact.title}</strong>.</p><p>Generated by the ${workspaceLabel} Executive Assistant.</p>`,
      attachments: [
        {
          filename: artifact.filename,
