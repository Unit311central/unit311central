...
import { buildEaWorkspaceDailyBrief } from "@/lib/ai-operating-assistant/workspace-packs";
...
export async function buildDailyExecutiveBrief(
  ...
  const packBrief = await buildEaWorkspaceDailyBrief(context);
  if (packBrief) return packBrief;
