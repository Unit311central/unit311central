...
export const ASSISTANT_TOOL_DEFINITIONS: AssistantToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "ABHI, Talanton Impact, OnwardAir, and Northstar. Automatically generate a professional multi-slide Board Meeting Pack / Board Deck (PowerPoint + PDF preview) from organisational data — cover, executive summary, previous actions, risk register, KPIs, financials, programme/fundraising (OnwardAir) or commercial/delivery (Northstar), team, and strategic discussion. Invoke whenever the user wants board meeting materials, board papers, a board pack, board deck, board presentation, or board report. Prefer this over generateReportPdf for any board pack/deck/papers request. Meeting date is resolved from the next scheduled Board Meeting, or relative phrases like tomorrow / next week when provided. Only pass meetingDate when the user gives an explicit YYYY-MM-DD.",
    parameters: {
      type: "object",
      properties: {
        meetingDate: {
