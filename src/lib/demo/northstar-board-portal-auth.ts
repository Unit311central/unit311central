...
import { NORTHSTAR_BOARD_PORTAL_PATH } from "@/lib/demo/northstar-board-portal-data";
import { PLATFORM_SESSION_COOKIE, readPlatformSessionToken } from "@/lib/platform-session-token";

export type NorthstarBoardPortalSession = {
  userId: string;
  username: string;
  displayName: string;
  userType: string;
...
export async function requireNorthstarBoardPortalAccess(): Promise<{
  session: NorthstarBoardPortalSession;
}> {
  ...
  if (allowed !== `/${NORTHSTAR_BOARD_PORTAL_PATH}`) {
    redirect("/board/login");
  }
