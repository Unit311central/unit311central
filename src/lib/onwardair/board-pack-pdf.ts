...
export async function buildOnwardAirBoardPackPdf(
  ...
  // Slide 1 — Cover
  {
    ...
    doc.setFontSize(11);
    setText(doc, C.muted);
    doc.text("OnwardAir", MARGIN, 32);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(28);
    setText(doc, C.navy);
    doc.text("Board Deck", MARGIN, 45);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(14);
    setText(doc, C.text);
  ...
  // Slide 7 — P&L
  {
    ...
    for (const [index, row] of data.pnl.rows.entries()) {
      ...
      setText(doc, C.navy);
      doc.text(formatOaBoardUsd(row.actual, true), colX[1]!, y + 1);
      setText(doc, C.muted);
      doc.text(formatOaBoardUsd(row.budget, true), colX[2]!, y + 1);
      setText(doc, row.variance < 0 ? C.subtleRed : C.green);
      doc.setFont("helvetica", "bold");
      doc.text(varianceText(row.variance), colX[3]!, y + 1);
      doc.setFont("helvetica", "normal");
      setText(doc, C.muted);
      doc.text(formatOaBoardUsd(row.priorYear, true), colX[4]!, y + 1);
      y += rowH;
    }
    setFill(doc, C.white);
    setDraw(doc, C.line);
  ...
  // Slide 9 — Commercial Performance (growth / pipeline visuals)
  {
    ...
    // Sponsorship
    {
      ...
      doc.setFontSize(10);
      setText(doc, C.muted);
      doc.text(`of ${formatOaBoardUsd(s.budget, true)} target`, x + 5, colY + 44);
      ...
      doc.setFontSize(18);
      setText(doc, C.navy);
      doc.text(formatOaBoardUsd(s.forecast, true), x + 5, colY + 108);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8);
      setText(doc, C.muted);
    ...
    // Events / WHX
    {
      ...
      setText(doc, C.muted);
      doc.text(
        `Events revenue YTD  ${formatOaBoardUsd(data.commercial.events.revenue, true)}`,
        x + 5,
        colY + 116,
      );
    }
