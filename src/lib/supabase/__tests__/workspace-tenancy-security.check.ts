 ...
 * Schema checks require DATABASE_URL/POSTGRES_URL (pg) or SUPABASE_ACCESS_TOKEN (Management API).
...
let schemaBackendLabel: string | null = null;
...
async function schemaScalar<T>(sql: string, label: string): Promise<T> {

  const result = await querySchemaScalar<T>(sql);

  if (!schemaBackendLabel) {

    schemaBackendLabel = result.backend;

    console.log(`[schema] Using ${result.backend}`);
...
async function checkInternalDefaultsRemoved(): Promise<void> {
  ...
  console.log(`[schema] Internal workspace_id defaults remaining: ${count}`);

  assert.equal(count, 0, "Internal workspace_id defaults must be removed (migration 142)");

}



async function checkFoundationRls(): Promise<void> {
  ...
  console.log(`[schema] Foundation tables with RLS enabled: ${count}/5`);

  assert.equal(count, 5, "All foundation tables must have RLS enabled (migration 143)");

}



async function checkPriorityDenyAll(table: string): Promise<void> {
...
async function main() {
  ...
  await checkInternalDefaultsRemoved();

  await checkFoundationRls();

  for (const table of PRIORITY_TABLES) {

    await checkPriorityDenyAll(table);
  ...
  if (!hasServiceRole) {
    ...
    console.log("\nPhase 1 schema checks complete (partial).");
  ...
  assert.ok(workspaces.length >= 2, "Need at least two Phase 1 workspaces in production DB");
