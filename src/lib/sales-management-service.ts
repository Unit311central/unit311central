...
export async function loadSalesQuotesForWorkspace(input: {
  ...
  return (data ?? []).map((row) => ({
    ...
    discountAmount: Number(row.discount_amount) || 0,
    pricingStyle: row.pricing_style === "scope_total" ? "scope_total" : "detailed",
    lineColumnVisibility: {
      showQuantity: row.line_column_visibility?.showQuantity !== false,
      showUnit: row.line_column_visibility?.showUnit !== false,
      showRate: row.line_column_visibility?.showRate !== false,
      showDiscount: row.line_column_visibility?.showDiscount !== false,
      showTax: row.line_column_visibility?.showTax !== false,
    },
    bankDetails: row.bank_details && typeof row.bank_details === "object" ? (row.bank_details as SalesQuote["bankDetails"]) : null,
