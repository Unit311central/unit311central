...
import {
  ...
  ensureWorkspaceOnboardingCompletedColumn,
...
export async function POST(request: NextRequest) {
  ...
  try {
    const applied = await ensureWorkspaceOnboardingCompletedColumn();
    if (!applied) {
      return NextResponse.json(
        {
          ok: false,
          migration: WORKSPACE_ONBOARDING_COMPLETED_MIGRATION_PATH,
