...
import {
  readOnwardAirPortalsContent,
  writeOnwardAirPortalsContent,
} from "@/lib/onwardair/portals-content-store";
...
export async function GET(request: NextRequest) {
  ...
  if (!session || !isOnwardAirPortalsAllowedUsername(session.username)) {
    return portalsJson({ error: "Authentication required." }, { status: 401 });
  }

  const content = await readOnwardAirPortalsContent();
  return portalsJson(
    {
      content,
      canEdit: isOnwardAirPortalsAdminUsername(session.username),
...
export async function PUT(request: NextRequest) {
  ...
  if (!isOnwardAirPortalsAdminUsername(session.username)) {
    return portalsJson({ error: "Admin access required." }, { status: 403 });
  }

  try {
    const body = (await request.json()) as { content?: unknown };
    const content = await writeOnwardAirPortalsContent(sanitizePortalsContent(body.content));
