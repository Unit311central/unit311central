...
export function WsEmpty({ message }: { message: string }) {
  return (
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm">
      ...
      <aside className="flex h-full w-full max-w-xl flex-col border-l border-white/10 bg-[#0b1524] shadow-2xl">
        <header className="flex items-start justify-between gap-3 border-b border-white/10 px-5 py-4">
          ...
          <button type="button" onClick={onClose} className={WsSecondaryButtonClass()}>
            Close
          </button>
        </header>
