...
export default function LogisticsDashboard({ onManageProviders }: LogisticsDashboardProps) {
  ...
  function handleAddPackage() {
    ...
    const shipment: LogisticsShipment = {
      ...
      carrierTrackingUrl: "https://www.example.com/track",
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-white/15 bg-white/[0.04] p-4 shadow-[0_24px_64px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl sm:p-6">
          ...
          <div className="mt-4 grid gap-3 rounded-xl border border-white/10 bg-[#0b1524]/60 p-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="flex items-end">
              <button
                ...
                disabled={
                  !newPackage.id.trim() || !newPackage.origin.trim() || !newPackage.destination.trim()
                }
        ...
        <div className="mt-4 flex flex-col gap-3 rounded-xl border border-sky-400/20 bg-sky-500/[0.07] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-sky-400/30 bg-sky-500/10">
              <Link2 className="h-4 w-4 text-sky-200" aria-hidden />
            </span>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-white">Shipping Providers</p>
              <p className="mt-0.5 text-xs leading-relaxed text-white/70">
                Connect couriers from the Provider Registry, continue setup, or manage mock
                connections. Unit311 Logistics remains available without APIs.
              </p>
            </div>
          </div>
          <button
            ...
            className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-sky-400/40 bg-sky-500/15 px-4 text-xs font-semibold text-sky-100 transition-colors hover:bg-sky-500/25"
          >
            Manage Providers
          </button>
        </div>
