...
export function FlightScenariosTab({ model, onSave, saving }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-white/10 p-4">
        <h3 className="text-sm font-semibold text-white">Video profile (recording vs live stream)</h3>
        <p className="mt-1 text-xs text-white/45">
          Camera max recording bitrate is separate from live transmission bitrate.
        </p>
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <label className="text-sm">
            <span className="text-white/45">Drone</span>
            <input
              className={WsInputClass()}
              value={draft.videoProfile.droneModel}
              onChange={(e) =>
                patch({
                  videoProfile: { ...draft.videoProfile, droneModel: e.target.value },
                })
              }
            />
          </label>
