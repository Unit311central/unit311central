...
import { QA_PAGE_LEVEL_ELEMENT } from "@/lib/qa-workspace/constants";
import {
  describeQaElement,
  ...
  return (
    <div data-qa-overlay className="pointer-events-none fixed inset-x-0 bottom-4 z-[80] flex justify-center px-4">
      <div
        ...
        className="pointer-events-auto flex flex-wrap items-center gap-2 rounded-xl border border-rose-400/40 bg-[#1a0b10]/95 px-3 py-2 text-xs text-rose-50 shadow-lg backdrop-blur"
      >
        <span className="font-semibold uppercase tracking-[0.12em] text-rose-200">QA Mode</span>
        <span className="text-rose-100/80">Click a highlighted element to capture a task.</span>
        <button
          type="button"
        ...
        >
          Page-level task ({QA_PAGE_LEVEL_ELEMENT})
        </button>
      </div>
    </div>
  );
