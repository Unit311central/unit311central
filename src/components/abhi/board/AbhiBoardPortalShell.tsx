...
import { ABHI_BOARD_NAV } from "@/lib/abhi/board-portal-data";
...
export function AbhiBoardPortalShell({ displayName, children }: Props) {
  ...
  return (
    <div className="flex min-h-screen bg-[#07111f] text-white">
      <aside className="flex w-64 shrink-0 flex-col border-r border-white/10 bg-[#0a1628] px-4 py-5">
        <div className="mb-6 space-y-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-white">ABHI Board</p>
      ...
      <main className="min-w-0 flex-1 overflow-auto px-6 py-6 sm:px-8">
        <div className="mb-6 text-xs text-white/40">ABHI · Board Portal · Confidential</div>
        {children}
      </main>
    </div>
  );
