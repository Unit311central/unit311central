...
import { OA_BOARD_NAV } from "@/lib/onwardair/board-portal-data";
...
export function OnwardAirBoardPortalShell({ displayName, children }: Props) {
  ...
  return (
    <div className="flex min-h-screen bg-[#061018] text-white">
      <aside className="flex w-64 shrink-0 flex-col border-r border-white/10 bg-[#0a1a22] px-4 py-5">
        <div className="mb-6 space-y-3">
          <OnwardAirLogoMark height={40} maxWidth={200} />
          <div>
            <p className="inline-flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/40">
              <Shield className="h-3 w-3" />
              Board Portal
            </p>
            <p className="mt-1 text-sm font-semibold text-white">OnwardAir Board</p>
      ...
      <main className="min-w-0 flex-1 overflow-auto px-6 py-6 sm:px-8">
        <div className="mb-6 text-xs text-white/40">
          OnwardAir · Board Portal · Confidential · Houston HQ
        </div>
        {children}
      </main>
    </div>
