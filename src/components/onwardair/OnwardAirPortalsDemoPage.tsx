...
const PORTALS_ADMIN_LOCK_KEY = "onwardair_portals_admin_lock";
...
export default function OnwardAirPortalsDemoPage() {
  const [content, setContent] = useState<OnwardAirPortalsEditableContent>(() =>
    defaultOnwardAirPortalsContent(),
  );
  ...
  const applyContent = useCallback((next: OnwardAirPortalsEditableContent, markDirty: boolean) => {
  ...
  const loadContent = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const response = await fetch("/api/onwardair/portals-content", {
        cache: "no-store",
      ...
      const data = (await response.json()) as {
        content?: OnwardAirPortalsEditableContent;
        canEdit?: boolean;
        username?: string;
        error?: string;
      };
  ...
  const saveContent = useCallback(async (payload?: OnwardAirPortalsEditableContent) => {
    ...
    <div className={cn(body.className, "min-h-[100dvh] bg-[#07111f] text-white")}>
      <div className="relative z-10 mx-auto flex w-full max-w-[1280px] flex-col px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        ...
        <footer className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5 text-[11px] text-white/40">
          <p>
            {SITE_NAME} · Confidential demonstration material for OnwardAir
          </p>
          <button
            type="button"
            onClick={() => void handleLogout()}
