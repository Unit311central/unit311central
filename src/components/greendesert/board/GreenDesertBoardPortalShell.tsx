...
import { GREENDESERT_BOARD_NAV } from "@/lib/greendesert/greendesert-board-portal-data";
...
export function GreenDesertBoardPortalShell({ displayName, children }: Props) {
    ...
    <>
      <div className="mb-6 space-y-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-white">{GREENDESERT_DISPLAY_NAME} Board</p>
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto">
        {GREENDESERT_BOARD_NAV.map((item) => (
          <Link
            key={item.id}
            href={item.href}
  ...
  return (
    <div className="flex min-h-[100dvh] bg-[#041208] text-white safe-area-pt safe-area-pb">
      {mobileNavOpen ? (
        <button
          type="button"
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-64 max-w-[85vw] flex-col border-r border-white/10 bg-[#071a0e] px-4 py-5 transition-transform duration-200 ease-out lg:static lg:z-auto lg:max-w-none lg:shrink-0 lg:translate-x-0",
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="safe-area-px flex h-14 shrink-0 items-center gap-3 border-b border-white/10 px-3 sm:px-5 lg:hidden">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-white">{GREENDESERT_DISPLAY_NAME} Board</p>
            <p className="truncate text-[10px] uppercase tracking-wide text-white/40">Board Portal</p>
          </div>
          <GreenDesertLogoMark height={28} maxWidth={120} />
        </header>
        <main className="min-w-0 flex-1 overflow-auto overflow-x-hidden px-4 py-5 sm:px-8 sm:py-6">
          <div className="mb-5 text-xs text-white/40">
            {GREENDESERT_DISPLAY_NAME} · Board Portal · Confidential
          </div>
          {children}
        </main>
      </div>
